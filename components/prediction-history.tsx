...
import { type Prediction, type PredictionType, getPredictions } from "@/app/actions/prediction-logger"
...
export function PredictionHistory() {
  ...
  const loadPredictions = async (type: PredictionType) => {
    setIsLoading(true)
    try {
      const data = await getPredictions(type, 30)
      setPredictions(data)
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-black/90 border border-white/10 text-white max-w-4xl w-[90vw] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
                    ...
                    <tbody>
                      {isLoading ? (
                        ...
                        predictions.slice(0, 10).map((prediction) => {
                          ...
                          return (
                            <tr key={prediction.id} className="border-t border-white/5 hover:bg-white/5">
