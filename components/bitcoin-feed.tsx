...
import { fetchBitcoinData } from "@/app/actions/fetch-market-data"
...
export function BitcoinFeed({ detailed = false }: BitcoinFeedProps) {
  ...
  // Fetch Bitcoin data and generate prediction
  useEffect(() => {
    const getBitcoinData = async () => {
      try {
        ...
        if (!predictionGenerated) {
          await generateBitcoinPrediction(marketData)
        }
      } catch (err) {
        ...
        const simulatedData = Array.from({ length: 20 }, (_, i) => {
          const value = baseValue + Math.random() * 2000 - 1000 + i * 20
          return {
            time: `${i}:00`,
  ...
  const generateBitcoinPrediction = async (marketData: any[]) => {
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="h-64 relative group">
        <div className="absolute inset-0 bg-gradient-to-b from-[#F7931A]/5 to-transparent rounded-xl opacity-50"></div>
        {isPredicting && (
      ...
      {predictionValue !== null && (
        <Card className="bg-black/40 border-white/10 overflow-hidden hover:border-[#F7931A]/30 transition-all duration-300">
          <CardContent className="p-3">
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 text-sm font-medium mb-1">
                  <Lightbulb className="h-4 w-4 text-[#F7931A]" />
                  AI Prediction (24h)
                </div>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <Card
                key={metric.metric}
                className="bg-black/40 border-white/10 overflow-hidden group hover:border-[#F7931A]/50 transition-all duration-300"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-[#F7931A]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
